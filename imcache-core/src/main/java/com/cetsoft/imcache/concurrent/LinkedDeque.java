...
/**
 * Linked list implementation of the Deque interface (not available in Java 5,
 * so not formally declared) where the link pointers are tightly integrated with
 * the element. Linked deques have no capacity restrictions; they grow as
 * necessary to support usage. They are not thread-safe; in the absence of
 * external synchronization, they do not support concurrent access by multiple
...
public class LinkedDeque<E extends Linked<E>> extends AbstractCollection<E> {
    ...
    abstract class AbstractLinkedIterator implements Iterator<E> {
        ...
        /**
         * Retrieves the next element to traverse to or <tt>null</tt> if there
