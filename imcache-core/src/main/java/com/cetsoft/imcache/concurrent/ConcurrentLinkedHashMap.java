 ...
 * exceeded its capacity. An entry is evicted from the map when the
 * <tt>weighted capacity</tt> exceeds its <tt>maximum weighted capacity</tt>
 * threshold. A {@link Weigher} instance determines how many units of capacity
 * that a value consumes. The default weigher assigns each value a weight of
 * <tt>1</tt> to bound the map by the total number of key-value pairs. A map
 * that holds collections may choose to weigh values by the number of elements
 * in the collection and bound the map by the total number of elements that it
 * contains. A change to a value that modifies its weight requires that an
 * update operation is performed on the map.
 * <p>
